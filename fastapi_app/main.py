...
@app.get("/")
async def read_root():
    logging.error("Welcome to Arshan's FastAPI App")
    return {"Hello": "Arshan"}
...
async def io_task():
    time.sleep(1)
    logging.error("io task is being performed.")
...
async def cpu_task():
    for i in range(1000):
        _ = i * i * i
    logging.error("cpu task. Time for a lunch break")
...
async def chain(response: Response):
    ...
    async with httpx.AsyncClient() as client:
        await client.get(
            f"http://{TARGET_TWO_HOST}:8000/error_test",
            headers=headers,
        )
